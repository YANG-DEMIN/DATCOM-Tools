...
s.tr = 1.0;
s.HINAX = 82.0;    % Longitudinal location of horizontal tail hinge axis.
...
s.blref = 85;      %lateral reference length. Wing span
s.rougfc = '1.0E-3';     %surface roughness factor

%% SYNTHS
s.xcg = 45.0;       % 后面要复写
s.zcg = 0.0;
s.xw = 40.0;         %longitudinal location of theoretical wing apex
s.zw = 1.0;         %vertical location of theoretical wing apex 
s.aliw = 0.0;       %wing root chord incidence angle measured from reference plane  new
s.xh = 80.0;        %longitudinal location of theoretical horizontal tail apex
...
s.xv = 80.0;
s.zv = 0.0;
s.xv_fin = 80.0;   %Longitudinal location of theoretical ventral fin apex
...
s.flap_f_type = 1.0;          % 1.0  Plain flaps
                            ...
                            % 8.0  Krueger
s.flap_delta = 0.0 : 5.0 : 20.0;
s.flap_tangent = 0.0522;    % PHETE   Tangent of airfoil trailing edge 
                            % Angle based on ordinates at x/c - 0.90 and 0.99
s.flap_tangent_p = 0.0391;   % PHETEP  Tangent of airfoil trailing edge 
...
s.flap_chord_balance = 0.5 * (s.flap_chord_in + s.flap_chord_out);
...
s.ail_type = 5.0;           % 1.0  Flap spoiler on wing
...
xh = s.xh - s.xcg;
Ah = 5.0;                           % 平尾展弦比
lambda_h = 1/2.8;                   % 平尾梢根比

c_aver = 0.5 * (s.W_croot + s.W_ctip);
s.kh = 0.5;                                  % 平尾力臂
s.Sh = s.kh * s.sref * c_aver / xh;          % 平尾面积
...
xv = s.xv - s.xcg;
Av = 1.5;                               % 垂尾展弦比
lambda_v = 1/2.8;                       % 垂尾梢根比

s.kv = 0.15;                                      % 垂尾力臂
s.Sv = s.kv * s.sref * s.blref / xv / 2;          % 垂尾面积

s.Vertical = 1.0;                                    %Flag of Vertical
s.V_semispan = 0.5 * sqrt(Av * s.Sv);            %SSPN
s.V_exp_semispan = s.V_semispan - 4.0;               %SSPNE
s.V_croot = 2 * s.Sv / (2 * s.H_semispan * (1+lambda_v));         %CHRDR Chord root
...
s.Vertical_Fin = 1.0;           %Flag of Vertical
...
s.ele_span_in = 0.1 * s.H_semispan;      % SPANFI  Span location of inboard end of flap or spoiler control
...
AR = 4 * s.W_semispan / (s.W_croot + s.W_ctip);
Lambda = s.W_sweep;
...
%         zz = spline(appo.alpha,appo.cl ./ appo.cd ,xx);
        plot(xx,yy,'LineWidth',2)
        hold on
        
        figure(3)
        plot(xx,L_D,'LineWidth',2)
